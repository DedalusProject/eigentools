...
evp = h5py.File("IVP_output/IVP_output_s1.h5","r")
df0 = h5py.File("ra1e5/snapshots/snapshots_s1.h5","r")
df = h5py.File("ra1e5/snapshots/snapshots_s26.h5","r")

ts_ra1e5 = h5py.File("timeseries/timeseries_s1/timeseries_s1_p0.h5","r")
...
growth = 184.65944709539858
model = ts_ra1e5['tasks/b_rms'][0,0,0]*np.exp(growth*ts_ra1e5['scales/sim_time'][:])
ax3 = mfig.add_axes(0,2,[0,0,1,1])
ax3.semilogy(ts_ra1e5['scales/sim_time'],ts_ra1e5['tasks/b_rms'][:,0,0], label='IVP')
ax3.semilogy(ts_ra1e5['scales/sim_time'],model, alpha=0.5, label='EVP')
ax3.legend()
ax3.set_ylim(6e-5,6e-1)
ax3.set_xlim(0,0.14)
ax3.set_xlabel("time")
